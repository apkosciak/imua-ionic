...
.controller 'StudentExpectationController', ['$stateParams', '$scope', '$location', 'student', 'current_user', 'ExpectationService', 'ProgressService', 'OrganizationService',
  ($stateParams, $scope, $location, student, current_user, ExpectationService, ProgressService, OrganizationService) ->
    ...
    $scope.viewExpectation = (user_expectation_id) ->
      $location.path("/app/user/#{student.id}/user_expectation/#{user_expectation_id}")
