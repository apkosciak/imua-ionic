(function() {
  angular.module('myApp').controller("OrgAdminDashboardController", [
    '$scope', 'OrganizationService', function($scope, OrganizationService) {
      $scope.current_organization = $scope.current_user.organization_name;
      $('input, textarea').placeholder();
      return OrganizationService.getOrganizationWithUsers($scope.current_user.organization_id).success(function(data) {
        ...
        if ($scope.organization.students.length === 0) {
          ...
          $scope.percent_students_with_one_activity = ((_.filter($scope.organization.students, function(student) {
            return student.semester_extracurricular_activities > 0;
          }).length / $scope.organization.students.length) * 100).toFixed(0);
        }
