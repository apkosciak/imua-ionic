...
.service 'OrganizationService', ['$http', '$q', 'CONSTANTS', ($http, $q,CONSTANTS) ->

  @all = ()->
    $http.get(CONSTANTS.API.base_url+'/api/v1/organization')

  @getOrganizationWithRoadmap = (orgId) ->
    $http.get(CONSTANTS.API.base_url+'/api/v1/organization/' + orgId + '/info_with_roadmap')

  @getOrganizationWithUsers = (orgId) ->
    $http.get(CONSTANTS.API.base_url+'/api/v1/organization/' + orgId + '/info_with_users')

  @getTimeUnits = (orgId) ->
    $http.get(CONSTANTS.API.base_url+'/api/v1/organization/' + orgId + '/time_units')

  @addOrganization = (name) ->
    $http.post(CONSTANTS.API.base_url+'/api/v1/organization', {name: name})
  ...
  @parseOrganizationWithUsers = (org, timeUnitId) ->
    ...
    for user in org.users
      user.square_avatar_url = user.square_avatar_url.substr(1)
