...
.service "UserServiceOrganizationService", ['$http', ($http) ->
  ...
  @all = (userId, time_unit_id) ->
    $http.get CONSTANTS.API.base_url+"/api/v1/users/#{userId}/user_service_organization?time_unit_id=#{time_unit_id}"
  ...
  @saveServiceOrganization = (user_service_organization) ->
    if user_service_organization.id
      return $http.put CONSTANTS.API.base_url+"/api/v1/user_service_organization/#{user_service_organization.id}", {user_service_organization: user_service_organization}

  @saveNewServiceOrganization = (new_service_organization) ->
      return $http.post CONSTANTS.API.base_url+"/api/v1/users/#{new_service_organization.user_id}/user_service_organization", {user_service_organization: new_service_organization, user_service_hour: new_service_organization.hours[0]}

  @saveServiceHour = (user_service_hour) ->
    if user_service_hour.id
      return $http.put CONSTANTS.API.base_url+"/api/v1/user_service_hour/#{user_service_hour.id}", {user_service_hour: user_service_hour}
    else
      return $http.post CONSTANTS.API.base_url+"/api/v1/users/#{user_service_hour.user_id}/user_service_hour", {user_service_hour: user_service_hour}
