...
.service 'CommentService', ['$http', '$q', 'CONSTANTS', ($http, $q, CONSTANTS) ->
  self = this
  ...
  @doSaveNewComment = (commentable_object_type, commentable_object_id, comment) ->
    $http.post CONSTANTS.API.base_url+"api/v1/#{commentable_object_type}/#{commentable_object_id}/comment",
      { comment: comment }

  @doSaveComment = (comment_id, comment) ->
    $http.put CONSTANTS.API.base_url+"api/v1/comment/#{comment_id}",
      { comment: comment }

  @doGetComments = (commentable_object_type, commentable_object_id) ->
    $http.get CONSTANTS.API.base_url+"api/v1/#{commentable_object_type}/#{commentable_object_id}/comments"

  @doDeleteComment = (comment_id) ->
    $http.delete CONSTANTS.API.base_url+"api/v1/comment/#{comment_id}"
